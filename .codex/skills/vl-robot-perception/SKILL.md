...
5. Always report the result as an automatic pre-check, not final grasp acceptance.
   ...
   - State whether the provider was `color_fixture`, `manual_region`, or `openai_vision`.
   - If `openai_vision` was skipped because no API key exists, say so plainly.
