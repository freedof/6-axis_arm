...
from src.perception.vl_region import estimate_region_3d, locate_openrouter_vision_region, region3d_to_dict
...
def main() -> None:
    if not _has_provider_key("openrouter_vision"):
        print("status: SKIPPED")
        print("reason: config/vl_providers.local.json is missing or openrouter_vision.api_key is not set.")
        return

    model_path = write_d435i_pick_scene_model(DEFAULT_D435I_PICK_MODEL)
    output_dir = DEFAULT_OUTPUT_DIR / "openrouter_vl_verify"
    observation = render_preview(model_path, output_dir, width=424, height=240, pose="scan_high")
    overlay = output_dir / "openrouter_vl_region_overlay.png"
    ...
    if region3d.valid_pixel_count < 20:
        raise RuntimeError(f"OpenRouter VL bbox has too few valid depth pixels: {region3d.valid_pixel_count}")
