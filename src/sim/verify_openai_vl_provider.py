...
from src.perception.vl_region import estimate_region_3d, locate_openai_vision_region, region3d_to_dict
...
def main() -> None:
    ...
    output_dir = DEFAULT_OUTPUT_DIR / "openai_vl_verify"
    observation = render_preview(model_path, output_dir, width=424, height=240, pose="scan")
    overlay = output_dir / "openai_vl_region_overlay.png"
    ...
    if region3d.valid_pixel_count < 20:
        raise RuntimeError(f"OpenAI VL bbox has too few valid depth pixels: {region3d.valid_pixel_count}")
