...
`ark_coding_vision` 会对每个视角做最多 `2` 次定位尝试，并在得到 bbox 后追加一次
VL 自检：让模型复核候选框是否覆盖目标主体、是否误框桌面/背景/夹爪。如果自检能给出修正
bbox，则使用修正后的 bbox；如果自检只判断不可靠但不能修正，则保留原 bbox，交给深度高度和
多视角一致性规则继续裁决。

真实外部 VL provider 还会使用更保守的多视角门槛：`openai_vision` / `ark_coding_vision`
至少需要 `2` 个视角通过深度高度和 3D 聚类检查，才允许进入抓取规划。只有 `1` 个视角通过时，
系统会报告感知不可靠，而不是直接执行抓取。
