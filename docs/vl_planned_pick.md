...
由估计出的 `cube_center` 生成四个关键抓取姿态：

```text
ready: 位于方块左后上方，夹爪保持更高、更接近竖直的出发姿态
...
`ready` 不再使用早期通用关节姿态，而是根据当前方块中心生成一个更适合 GIF 开场的高位
竖直姿态：夹爪 TCP 大约高于桌面 `0.177 m`，避免第一帧看起来像夹爪贴在桌面上。
当前抓取点会夹住方块上半部，而不是让夹爪中心下降到方块几何中心。按当前简化夹爪
几何，抓取姿态下手指最低点约高于桌面 `0.055 m`。简化夹爪现在使用较短的
finger 主体，并让高摩擦 pad 与 finger 上下平齐。
...
当前抓取 GIF 使用 MuJoCo 动力学仿真：方块是带 `freejoint` 的动态物体，具有质量、
接触、摩擦和接触求解参数；机械臂和夹爪通过 position actuator 跟踪目标。夹爪在闭合
和抬升时出现的晃动通常来自接触力、摩擦、物体惯性和 actuator 伺服跟踪的组合。
...
对外部真实 VL provider，当前多视角融合要求至少 `2` 个视角通过检查后才进入抓取规划；
只有单个视角通过时会被视为感知不够可靠。`color_fixture` 和 `codex_vision` 仍可用于
本地链路验证和交互式验收。
...
每个视角最多会定位重试 `2` 次，并进行一次 VL 自检/修正。自检结果只是辅助，最终仍由深度高度、
多视角聚类和“外部 VL 至少 2 个视角通过”的规则决定是否进入抓取规划。
