...
- The gripper-equipped model preserves `tool0` and adds `gripper_tcp` for
  ...
  finger body and a longer high-friction pad so the pad, not the long finger
  body, is the primary contact area during cube lifting.
...
- `target_3d` planned grasping is documented in `docs/vl_planned_pick.md`.
  ...
  simplified gripper geometry, the finger bottom is about `0.038 m` above the
